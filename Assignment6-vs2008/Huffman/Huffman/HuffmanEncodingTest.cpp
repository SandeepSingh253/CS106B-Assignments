...
void runCompress() {
	while (true) {
		...
		string inname = getLine("File to compress: ");
		ifbstream infile(inname);
		if (infile.fail()) {
		...
		string outname = getLine("Filename for compressed output: ");
		ofbstream outfile(outname);
		if (outfile.fail()) {
			cout << "Couldn't open file." << endl;
			continue;
		}
		...
		/* Compress the file. */
		cout << "Compressing... this may take a while..." << endl;
		compress(infile, outfile);
		
		/* Report statistics. */
		cout << "Compressed file written to " << outname << endl;
...
void runDecompress() {
	while (true) {
		...
		string inname = getLine("Compressed file: ");
		ifbstream infile(inname);
		if (infile.fail()) {
			cout << "Couldn't open file." << endl;
			continue;
		...
		string outname = getLine("Filename for unpacked output: ");
		ofbstream outfile(outname);
		if (outfile.fail()) {
			cout << "Couldn't open file." << endl;
			continue;
		...
		cout << "Decompressed file written to " << outname << endl;
		getLine("Press ENTER to continue...");
		break;
	}
}
 ...
 * --------------------------------------------------------
 * Utility function to compare two files to see if they are
 * equal to one another.
 */
void compareFiles(){
	while (true) {
	    /* Get the name of the input file. */
    	string inname = getLine("First file to check:  ");
		ifbstream first(inname);
		if (first.fail()) {
			cout << "Couldn't open file." << endl;
			continue;
		}
		...
		/* Get the name of the output file. */
		inname = getLine("Second file to check: ");
		ifbstream second(inname);
		if (second.fail()) {
			cout << "Couldn't open file." << endl;
			continue;
		}
   	
   		/* If the files have different size, they can't match. */
		if (first.size() != second.size()) {
			cout << "! Files not identical: Sizes are not the same." << endl;
		} else {
			bool success = true;
			char firstCh, secondCh;
    
			/* Note the use of non-short-circuiting AND. */
			while (first.get(firstCh).good() & second.get(secondCh).good()) {
				if (firstCh != secondCh) {
					cout << "! Files not identical: Character mismatch." << endl;
					success = false;
					break;
				}
			}
	    
			/* If we made it through successfully, report success. */
			if (success) {
...
void displayMenu() {
	...
	cout << setw(2) << COMPARE_FILES << ": Check if two files are identical." << endl;
	cout << QUIT << ": Quit" << endl;
}
...
void testEnvironment() {
  /* Create a char with a negative value. */
  char negative = '\xFF';

  /* Cast it to an integer. */
  int value = negative;

  /* Confirm that the value is positive. */
  if (value != 0xFF) {
    cerr << "============= ENVIRONMENT ERROR =============" << endl;
    cerr << "Something is wrong with this project setup." << endl;
    cerr << "This is our fault!  Please contact Keith or" << endl;
    cerr << "Zach as soon as possible to let them know" << endl;
